  ...
  return (
    <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-8 sm:px-6">
          ...
          return (
            <li key={line.productId} className="flex gap-4 border-b border-zinc-200 pb-4 dark:border-zinc-800">
              ...
              <div className="flex flex-1 flex-col gap-1">
                ...
                <span className="text-xs text-zinc-500">{line.packageSize}</span>
                <div className="flex items-center gap-2 text-sm text-zinc-600 dark:text-zinc-400">
                  {unitPrice !== null ? formatMoney(unitPrice) : "Quote required"}
                  {appliedTier && <Badge tone="brand">Bulk price</Badge>}
                </div>
                {issue && <span className="text-xs text-red-600 dark:text-red-400">{issue}</span>}
      ...
      <div className="mt-6 flex flex-col items-end gap-4">
        ...
        <p className="text-xs text-zinc-500">Delivery fee is calculated at checkout based on your state.</p>
        {hasBlockingIssue ? (
          <div className="flex flex-col items-end gap-2">
            <p className="text-sm text-red-600 dark:text-red-400">Resolve the issues above before checking out.</p>
            <Link href="/quote" className="text-sm font-medium text-brand-primary-dark underline dark:text-brand-primary">
