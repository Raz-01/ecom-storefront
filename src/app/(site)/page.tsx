...
export default async function Home() {
  ...
  return (
    <main className="flex flex-1 flex-col">
      <section className="border-b border-zinc-200 bg-gradient-to-b from-brand-primary/5 to-transparent dark:border-zinc-800">
        <div className="mx-auto flex max-w-6xl flex-col items-center gap-4 px-4 py-16 text-center sm:px-6">
          <p className="text-sm font-medium uppercase tracking-wide text-brand-primary-dark dark:text-brand-primary">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {categories.map((category) => (
            <Link
              ...
              className="rounded-lg border border-zinc-200 p-4 text-center text-sm font-medium hover:border-brand-primary hover:text-brand-primary-dark dark:border-zinc-800 dark:hover:text-brand-primary"
            >
              {category.name}
            </Link>
          ))}
        ...
        <div className="mx-auto grid max-w-6xl grid-cols-1 gap-8 px-4 py-12 sm:grid-cols-3 sm:px-6">
          {TRUST_POINTS.map((point) => (
            <div key={point.title}>
              ...
              <p className="text-sm text-zinc-600 dark:text-zinc-400">{point.body}</p>
            </div>
          ))}
        </div>
      </section>
