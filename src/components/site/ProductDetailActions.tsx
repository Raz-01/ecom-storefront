    ...
    <div className="flex flex-col gap-3">
      {resolution.kind === "quote_required" ? (
        <div className="rounded-md border border-brand-primary/30 bg-brand-primary/5 p-3 text-sm">
          <p className="font-medium text-brand-primary-dark dark:text-brand-primary">Quantities of {product.bulkQuoteThreshold}+ need a custom quote.</p>
        ...
        <p className="text-xl font-semibold">
          ...
          {resolution.appliedTier && <span className="ml-2 text-sm font-normal text-brand-primary-dark dark:text-brand-primary">Bulk price applied</span>}
        </p>
      )}
      ...
      {product.minOrderQuantity > 1 && <p className="text-xs text-zinc-500">Minimum order: {product.minOrderQuantity}</p>}
      {!outOfStock && product.stock <= 10 && <p className="text-xs text-amber-600 dark:text-amber-400">Only {product.stock} left in stock.</p>}
    </div>
  );
}
